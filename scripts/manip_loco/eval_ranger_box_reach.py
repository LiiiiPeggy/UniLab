...
def main() -> None:
    ...
    # Accumulators, split by reachable vs extended
    stats = {"reachable": _new_stats(), "extended": _new_stats(), "all": _new_stats()}
    n_episodes = 0
    obs = wrapped.reset()[0]
    ...
    with torch.inference_mode():
        while n_episodes < args.episodes:
            ...
            ep_success_hold = False
            ep_joint_limit = False
            ep_sat_count = 0
            ep_steps = 0
            base_start = env._backend.get_base_pos()[:, :2].copy()

            # Determine reachable vs extended per env from goal distance
            goals = env.world_ee_goal.copy()
            armbase = env.armbase_pos_world.copy()
            goal_dist = np.linalg.norm(goals - armbase, axis=1)
            is_reachable = goal_dist < 0.5
            ...
            for i in range(args.num_envs):
                key = "reachable" if is_reachable[i] else "extended"
                s = stats[key]
                _accum(
                    s,
                    ep_success_once[i],
...
def _accum(s, once, hold, dists, final, mind, base_disp, joint_lim, sat, steps):
    s["n"] += 1
...
def _report(stats, num_envs):
    ...
    for key in ("reachable", "extended", "all"):
        ...
        print(f"  base_disp mean {f['base_disp_mean']:.3f} m")
        print(
